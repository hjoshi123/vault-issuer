...
Port the controller serves its liveness and readiness probes on.
...
Elect a single active controller when more than one replica runs.
#### **leaderElection.leaseDuration** ~ `string`
> Default value:
> ```yaml
> 15s
> ```

How long a lease is held before another replica may take over.
#### **leaderElection.renewDeadline** ~ `string`
> Default value:
> ```yaml
> 10s
> ```

How long the holder retries renewing its lease before giving it up.
### Deployment
